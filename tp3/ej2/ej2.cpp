    ...
    for(int i = 0; i < latencias.size(); i++) {
        for(int j = 0; j < latencias.size(); j++) {
            for(int k = 0; k < latencias.size(); k++) {
                ...
                if (latencias[j][k] > latencias[j][i] + latencias[i][k])
                    return false; 
            }
        }
    }
   ...
   for(int i = 0; i < latencias.size(); i++) {
        for(int j = 0; j < latencias.size(); j++) {
            for(int k = 0; k < latencias.size(); k++) {
                ...
                if (latencias[j][k] == latencias[j][i] + latencias[i][k] && (j != i && i != k)) // nos fijamos que j sea distinto de i, e i distinto de k para no caer en distancias de un nodo a si mismo
                    latencias[j][k] = -1;
            }
        }
    }
...
int main() {
    ...
    for (int i = 0; i < cantTests; i++) {
        ...
        bool res = esFloydWarshall(latencias);
